...
export class ForceGraphComponent implements OnInit {
  ...
  constructor(
    ...
    public graphRef: MatDialogRef<FoodwebhomeComponent>,
    @Inject(MAT_DIALOG_DATA) public data: {nodes: Nodes[],
    links: Connections[]}) {
      this.getScreenSize();
  }
  ...
  loadForceDirectedGraph(): void {
    ...
    const simulation = d3.forceSimulation(node2)
      .force('x', d3.forceX(width))
      .force('y',  d3.forceY(height))
      .force('link', d3.forceLink(link2).id((d) => d[key]).distance(140).strength(1))
    ...
    node.append('text')
      .attr('x', d => Math.sqrt(d.count / this.radius) + 7)
      .attr('y', '0.31em')
      .text(d => d.id)
      .clone(true).lower()
      .attr('fill', 'none')
  ...
  linkArc(d: { source: { x: number; y: number; count: any; radius: any };
    target: { x: number; y: number; count: any; radius: any; }; }): string {
    ...
    const sinTheta = (Math.sqrt ((d.source.count) / d.source.radius) + 4) * Math.sin(theta);
    const cosTheta = (Math.sqrt ((d.source.count) / d.source.radius) + 4) * Math.cos(theta);
    const sinPhi = (Math.sqrt ((d.target.count) / d.target.radius) + 4) * Math.sin(phi);
    const cosPhi = (Math.sqrt ((d.target.count) / d.target.radius) + 4) * Math.cos(phi);
